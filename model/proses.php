...
elseif (isset($_POST['add_guru'])) {
	...
	if ($nis==="" || $email==="" || $pwd==="" || $nama==="" || $jk==="" || $sklh==="") {
		header("location:../add_guru&st=4");
	}else {
		if ($pwd !== "$pwd_cek") {
			header("location:../add_guru&st=5");
		} else {
				...
				if ($cek_nis==0) {
					if ($statement= $conn->prepare($sql_user) AND $statement1 = $conn->prepare($sql_detail)) {
						...
						if ($statement->execute() && $statement1->execute()) {
							header("location:../add_guru&st=1");
						} else {
							header("location:../add_guru&st=2");
						}
					} else {
						header("location:../add_guru&st=2");
					}
				} else {
					header("location:../add_guru&st=6");
				}
				$conn->close();
			} else {
				header("location:../add_guru&st=3");
			}
		}
	}
...
elseif (isset($_POST['edit_guru'])) {
	...
	if ($nis==="" || $id==="" || $nama==="" || $jk==="" || $sklh==="") {
		header("location:../guru&id=$id&st=4");
	}else {
		if ($statement= $conn->prepare($sql_detail)) {
				...
				if ($statement->execute()) {
					header("location:../guru&id_guru=$id&st=1");
				} else {
					header("location:../guru&id_guru=$id&st=2");
				}
			} else {
				header("location:../guru&id_guru=$id&st=2");
			}
...
elseif (isset($_GET['del_guru'])) {
	$id = mysqli_real_escape_string($conn, $_GET['del_guru']);
	...
	if ($stmt= $conn->prepare($sql_d) AND $stmt1 = $conn->prepare($sql_u)) {
		...
		if ($stmt->execute() && $stmt1->execute()) {
			header("location:../guru&st=3");
		} else {
			header("location:../guru&st=5");
		}
	} else {
		header("location:../guru&st=5");
	}
